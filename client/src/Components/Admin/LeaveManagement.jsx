...
const LeaveManagement = () => {
  ...
  const [type, setType] = useState("All Leave");
  const [leaveHistory, setLeaveHistory] = useState([]);

  const fetchRoute = (type) => {
    switch (type) {
      case "All Leave":
        return "fetch-leave-history";
      case "Pending Leave":
        return "fetch-pending-leave-history";
      case "Approved Leave":
        return "fetch-approved-leave-history";
      case "Rejected Leave":
        return "fetch-rejected-leave-history";
      case "Cancelled Leave":
        return "fetch-cancelled-leave-history";
      default:
        return "fetch-leave-history";
    }
  };

  const fetchLeaveHistory = (type) => {
    axios
      .get(`http://localhost:3000/${fetchRoute(type)}`)
      .then((res) => {
